...
int ConnectionCheck(char* ipAddr, unsigned int port, struct RBCP_HEADER* sendHeader) {
    ...
    sendHeader->address_0 = 0;
    sendHeader->address_1 = 0;
    sendHeader->address_2 = 0;
    sendHeader->address_3 = 0;
    sendHeader->data_0 = 0;
    sendHeader->data_1 = 0;
    sendHeader->data_2 = 0;
    ...
    while(numReTrans < 5) {
        ...
        timeout.tv_sec  = 1;
        timeout.tv_usec = 0;
        int n = select(sock+1, &setSelect, NULL, NULL, &timeout);
        if (n == 0) {
            puts("***** Timeout ! *****");
            sendHeader->id++;
            memcpy(sndBuf,sendHeader, sizeof(struct RBCP_HEADER));
